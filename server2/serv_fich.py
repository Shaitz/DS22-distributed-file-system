...
def session(s):
    ...
    while True:
        ...
        if message.startswith( szasar.Command.User ):
            ...
            s_server2 = socket.socket( socket.AF_INET, socket.SOCK_STREAM )
            s_server2.connect( ('', SERVER2_PORT) )
            s_server2.sendall(str(MESSAGE_ID).encode() + 'ç'.encode() + message.encode() + '|'.encode() + filedata)
            new_message = s_server2.recv(1024).decode()
            s_server2.close()
            print ("Child: message received: " + new_message + "")
...
if __name__ == "__main__":
    ...
    s_server2hb.bind(("", SERVER2HEARTBEAT_PORT))
    ...
    while True:
        ...
        if PRIMARY:
            try:
                readable, writable, exceptional = select.select([s, s_server2, s_server2hb], [], [], 0.5)
            except select.error:
                continue
            ...
            while True:
                ...
                if timeout < 1 and not CANDIDATE:
                    ...
                    s_server3 = socket.socket( socket.AF_INET, socket.SOCK_STREAM )       
                    s_server3.connect( ('', SERVER3_PORT) )
                    s_server3.sendall(str(CHALLENGE_MSG_ID).encode() + 'ç'.encode() + str(SERVER).encode())
                    s_server3.close()
                    CHALLENGE_MSG_ID -= 1
                    timeout = 15
                ...
                for sock in readable:
                    ...
                    elif sock == s_server2hb:
                        (dialog, address) = s_server2hb.accept()
                        message_complete = dialog.recv(4096).decode()
